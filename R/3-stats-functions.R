...
#'
#'@inheritParams computeVectorMeasure
#'@param w (optional) numerical vector of weights the same length as \code{x}
...
trimean <- function(
  ...
  if(!is.null(i)){x = x[i]}
  #median
  medianValue = stats::median(x = x, na.rm = na.rm)
  #midhinge
...
#'
#'@inheritParams computeColMeasures
#'@param w numerical vector of weights. Its length must match the number of rows
#'in \code{x}
...
colWeightedSums <- function(
  ...
  #check
  if(isTRUE(missing(w) || is.null(w))) {w = rep(x = 1, times = nrow(x))}
