...
### Random Signatures
To generate random signatures, i.e. randomly generated vectors of the same size as `i`, we just need to change the `sampling = 'rndsig'`.
...
A final available option is to generate random signatures by using all possible elements of `x` after removing the value from `i`. For this scenario, we just need to change the `sampling = 'rndsigsub'`.
